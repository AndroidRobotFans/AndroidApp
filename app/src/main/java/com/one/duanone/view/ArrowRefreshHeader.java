...
import com.one.duanone.interfaces.BaseRefreshHeader;

import java.util.Date;

/**
 * PC: Masterr_Robot.
 * Created by DKL on 2016/8/2  22:22.
 */
public class ArrowRefreshHeader extends LinearLayout implements BaseRefreshHeader {

    private LinearLayout mContainer;
    private ImageView mArrowImageView;//显示下拉箭头的Image
    private SimpleViewSwitcher mProgressBar;
    private TextView mStatusTextView; //显示正在刷新的textView
    private int mState = STATE_NORMAL;

    private TextView mHanderTimeView;
    ...
    private void initView() {
        ...
        progressView.setIndictorColor(0xFFB5B5B5);
        progressView.setIndicatorID(-1);
        ...
        mHanderTimeView = (TextView) findViewById(R.id.last_refresh_time);
        measure(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        mMeasuredHeight = getMeasuredHeight();

    }

    public void setProgressBar(int style) {
        if (style == -1) {
    ...
    public void setState(int state) {
        ...
        switch (state) {
            case STATE_NORMAL:
                if (mState == STATE_RELEAS_TO_REFRESH) {
    ...
    public void refreshComplete() {
        mHanderTimeView.setText(friendlyTime(new Date()));
        setState(STATE_DONE);
    ...
    public void onMove(float delta) {
        if (getVisibleHeight() > 0 || delta > 0) {
            ...
            if (mState <= STATE_RELEAS_TO_REFRESH) {//未处于刷新状态更新箭头
                if (getVisibleHeight() > mMeasuredHeight) {
                    setState(STATE_RELEAS_TO_REFRESH);
                } else {
